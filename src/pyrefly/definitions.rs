...
pub struct Definitions {
    ...
    /// LIFEGUARD: If the module imports importlib
    pub has_importlib: bool,
    /// LIFEGUARD: If the module imports import_module from importlib
...
impl Definitions {
    ...
    pub fn inject_builtins(&mut self) {
        ...
        // LIFEGUARD: We don't use extra_builtins
        // self.import_all
    ...
    pub fn inject_implicit_globals(&mut self) {
        // LIFEGUARD: We have not imported the new globals.rs with ImplicitGlobals since it has
        // gotten more typing-focused.
...
impl<'a> DefinitionsBuilder<'a> {
        ...
        match x {
            Stmt::Import(x) => {
                for a in &x.names {
                    ...
                    if a.name.id == "importlib" || a.name.id.starts_with("importlib.") {
                        self.inner.has_importlib = true;
                    }
                    match &a.asname {
                        None => self.add_name(
            ...
            Stmt::ImportFrom(x) => {
                ...
                for a in &x.names {
                    if name.as_ref().is_some_and(|m| m.as_str() == "importlib")
                        && &a.name.id == "import_module"
                    {
                        self.inner.has_import_module = true;
                    }
                    if &a.name == "*" {
            ...
            Stmt::Assign(x) => {
                ...
                let import_module_state =
                    ImportlibState::new(self.inner.has_importlib, self.inner.has_import_module);
                for t in &x.targets {
                    if let Expr::Call(call) = &*x.value
                        && let Some(imported_module) = import_module_state.match_call(call)
                    {
                        let Expr::Name(target) = t else {
                            continue;
                        };
            ...
            Stmt::Expr(StmtExpr { value, .. }) => {
                if self.handle_import_module(value) {
                    return;
                }
    ...
    fn handle_import_module(&mut self, stmt: &Expr) -> bool {
